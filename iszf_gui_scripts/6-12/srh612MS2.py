...
class Srh612Ms2():
    ...
    def initDataTable(self, srhFits, frequencyChannel = [], phaseCorrect = True, amplitudeCorrect = False):
        ...
        for freqInd in range(freqNumber):
            ...
            for scan in range(srhFits.dataLength):
                ...
                uv[:,0,0] = srhFits.visRcp[freq,scan,:visibilityNumber].copy()
                uv[:,0,1] = srhFits.visLcp[freq,scan,:visibilityNumber].copy()
                uv_corr = uv.copy()
                if phaseCorrect:
    ...
    def initAntennaTable(self, srhFits):
        ...
        for ant in range(128):
            ...
            self.antennaTable.col('MOUNT')[ant] = 'ALT-AZ'
        ...
        for ant in range(64):
            ...
            self.antennaTable.col('MOUNT')[128 + ant] = 'ALT-AZ'
    ...
    def initFeedTable(self, srhFits, frequencyChannel = []):
        self.feedTable.addrows(192)
        for ant in range(192):
