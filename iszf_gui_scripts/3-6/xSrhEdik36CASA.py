...
class SrhEdik36(QtWidgets.QMainWindow):#MainWindow):
...
#        self.srhFits.changeNorthPhase(self.nLcpPhaseCorrection, self.nRcpPhaseCorrection)
    ...
    def showImage(self):
        ...
        if self.indexOfImageType == 0 or self.indexOfImageType == 4 or self.indexOfImageType == 5:
            ...
            if self.autoscaleButton.isChecked():
                self.lcpCanvas.imshow(NP.angle(self.srhFits.uvLcp[self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8,self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8]), NP.min(NP.angle(self.srhFits.uvLcp)), NP.max(NP.angle(self.srhFits.uvLcp)))
                self.rcpCanvas.imshow(NP.angle(self.srhFits.uvRcp[self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8,self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8]), NP.min(NP.angle(self.srhFits.uvRcp)), NP.max(NP.angle(self.srhFits.uvRcp)))
            else:
                self.lcpCanvas.imshow(NP.angle(self.srhFits.uvLcp[self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8,self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8]) *self.imageScale + self.imageOffset, NP.min(NP.angle(self.srhFits.uvLcp)), NP.max(NP.angle(self.srhFits.uvLcp)))
                self.rcpCanvas.imshow(NP.angle(self.srhFits.uvRcp[self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8,self.uvSize//2-self.uvSize//8:self.uvSize//2+self.uvSize//8]) *self.imageScale + self.imageOffset, NP.min(NP.angle(self.srhFits.uvRcp)), NP.max(NP.angle(self.srhFits.uvRcp)))
    ...
    def onNorthLcpPhaseSlopeChanged(self, value):
        self.srhFits.nSlopeLcp[self.currentFrequencyChannel] = value
        self.srhFits.buildNPhase()
        if (self.imageUpdate):
            self.buildImage()
            self.showImage()

    def onNorthRcpPhaseSlopeChanged(self, value):
        self.srhFits.nSlopeRcp[self.currentFrequencyChannel] = value
        self.srhFits.buildNPhase()
        if (self.imageUpdate):
            self.buildImage()
            self.showImage()
    ...
    def onCalculatePhaseCurrent(self):
        # self.srhFits.baselines = 5
        self.srhFits.solarPhase(self.srhFits.frequencyChannel)
    ...
    def onFrequencyChannelChanged(self, value):
        ...
        self.nLcpPhaseSlopeSpin.setValue(self.srhFits.nSlopeLcp[self.currentFrequencyChannel])
        self.nRcpPhaseSlopeSpin.setValue(self.srhFits.nSlopeRcp[self.currentFrequencyChannel])
        # self.nPhaseStairLcp.setValue(self.srhFits.nLcpStair[self.currentFrequencyChannel])
        # self.nPhaseStairRcp.setValue(self.srhFits.nRcpStair[self.currentFrequencyChannel])
    ...
    def onAutoscale(self, value):
        if (self.imageUpdate):
            ...
            if self.indexOfImageType == 1:
                ...
                self.vminRcp = self.rcpData.min()
                self.vmaxRcp = self.rcpData.max()
            self.showImage()
...
#                            'threshold' : '5mJy'}
    ...
    def onMakeClean(self):
        ...
        tclean(vis = self.cleanParams['vis = '],
               ...
               # scales = self.cleanParams['scales = '])
        ...
        self.casaLcpData += (self.srhFits.lcpShift[self.srhFits.frequencyChannel]/NP.count_nonzero(self.srhFits.uvLcp))
        self.casaRcpData += (self.srhFits.rcpShift[self.srhFits.frequencyChannel]/NP.count_nonzero(self.srhFits.uvRcp))
...
#        os.system('casa -c ' + setjy_script.replace(' ', '\ '))
    ...
    def __init__(self, parent=None):
        ...
        self.ewAntennaNumber = 97
        self.nAntennaNumber = 31
...
#        
        ...
        self.calculatePhaseButton = QtWidgets.QPushButton('Calculate Cal (all)', self)
        self.calculatePhaseButton.clicked.connect(self.onCalculatePhase)
        
        self.calculatePhaseAllButton = QtWidgets.QPushButton('Calculate Cal (current)', self)
        self.calculatePhaseAllButton.clicked.connect(self.onCalculatePhaseCurrent)
        ...
        self.useRLDifButton = QtWidgets.QPushButton('Use RLdif file', self)
        self.useRLDifButton.setCheckable(True)
        self.useRLDifButton.setChecked(True)
        self.useRLDifButton.clicked.connect(self.onUseRLDif)
...
#        self.casaWidget.layout.addWidget(self.casaRightCanvas,5,1)
        ...
        self.tab2.layout.addWidget(self.sPhaseStairLcp, 0, 0)
        self.tab2.layout.addWidget(self.sPhaseStairRcp, 1, 0)
        self.tab2.layout.addWidget(self.phaseCorrectButton, 0, 1)
        self.tab2.layout.addWidget(self.amplitudeCorrectButton, 1, 1)
        self.tab2.layout.addWidget(self.calculatePhaseButton, 0, 2)
        self.tab2.layout.addWidget(self.calculatePhaseAllButton, 1, 2)
...
#        self.tab2.layout.addWidget(self.sAmpCoefSpin, 1, 7)
        ...
        self.tab4.layout.addWidget(self.nLcpPhaseSlopeSpin, 1, 0)
...
#            self.scan.setValue(self.currentScan)
            ...
            self.nLcpPhaseCorrection = NP.zeros(self.nAntennaNumber)
            self.nRcpPhaseCorrection = NP.zeros(self.nAntennaNumber)

            self.ewPhaseCoefsLcp = NP.zeros((self.srhFits.freqListLength, 97))
            self.ewPhaseCoefsRcp = NP.zeros((self.srhFits.freqListLength, 97))
            self.nPhaseCoefsLcp = NP.zeros((self.srhFits.freqListLength, 31))
            self.nPhaseCoefsRcp = NP.zeros((self.srhFits.freqListLength, 31))
            
            self.ewLcpPhaseSlope = NP.zeros((self.srhFits.freqListLength))
            self.ewRcpPhaseSlope = NP.zeros((self.srhFits.freqListLength))
            self.nLcpPhaseSlope = NP.zeros((self.srhFits.freqListLength))
            self.nRcpPhaseSlope = NP.zeros((self.srhFits.freqListLength))
...
#            self.rcpMaxCanvas.plot(self.rcpMaxTrace)
    ...
    def saveAsFits(self, saveName):
        if self.indexOfImageType == 3:
            nameParts = saveName.split('.') 
            fitsName = ('%s_%03d.%s' % (nameParts[0], self.srhFits.freqList[self.currentFrequencyChannel]/1e3, nameParts[1]))
            print(fitsName)
            ...
            pHeader['DATE-OBS']     = self.srhFits.hduList[0].header['DATE-OBS'] + 'T' + phaseEdit.timeList.itemText(self.currentScan)
            pHeader['T-OBS']     = self.srhFits.hduList[0].header['DATE-OBS'] + 'T' + phaseEdit.timeList.itemText(self.currentScan)#self.srhFits.hduList[0].header['TIME-OBS']
            pHeader['INSTRUME']     = self.srhFits.hduList[0].header['INSTRUME']
            pHeader['ORIGIN']       = self.srhFits.hduList[0].header['ORIGIN']
            pHeader['OBS-LAT']      = self.srhFits.hduList[0].header['OBS-LAT']
            pHeader['OBS-LONG']     = self.srhFits.hduList[0].header['OBS-LONG']
            ...
            pHdu = fits.PrimaryHDU(self.lcpData, header=pHeader);
            hduList = fits.HDUList([pHdu]);
            hduList.writeto(savePath + '_LCP.' + saveExt, overwrite=True);
            hduList.close();
            
            pHdu = fits.PrimaryHDU(self.rcpData, header=pHeader);
            hduList = fits.HDUList([pHdu]);
            hduList.writeto(savePath + '_RCP.' + saveExt, overwrite=True);
        ...
        #         self.scan.setValue(scan)
            scan = self.currentScan
            nameParts = saveName.split('.') 
            fitsName = ('%s_%04d.%s' % (nameParts[0], scan, nameParts[1]))
            print(fitsName)
            if self.indexOfImageType == 3:
                self.lcpData /= NP.max(self.lcpData)
                self.rcpData /= NP.max(self.rcpData)
