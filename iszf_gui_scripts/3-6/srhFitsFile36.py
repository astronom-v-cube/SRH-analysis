...
class SrhFitsFile():
    ...
    def loadGains(self, filename):
        ...
        self.AntAmpRcp = NP.array(currentGains['nAmpRcp'])
...
#        self.ewAntPhaRcp[freqChannel] = NP.angle(self.ew_gains_rcp)
    ...
    def buildNPhase(self):
        ...
        for j in range(self.antNumberN):
                newLcpPhaseCorrection[j] += NP.deg2rad(-self.nSlopeLcp[self.frequencyChannel] * (j + 1)) 
                newRcpPhaseCorrection[j] += NP.deg2rad(-self.nSlopeRcp[self.frequencyChannel] * (j + 1))
